...
def get_patches(image_array:cp.ndarray, kernel_size:int, stride:int=1) -> cp.ndarray:
    patches = []
    ...
    return cp.array(patches)

def create_gaussian_kernel(kernel_size:int, sigma=None) -> cp.ndarray:
    if sigma is None:
        sigma = 0.3 * ((kernel_size - 1) * 0.5 - 1) + 0.8   # Scaling sigma based on kernel size, may need to adjust the coefficients
    kernel = cp.fromfunction(lambda x, y: (1/(2*cp.pi*sigma**2)) * cp.exp(-((x-(kernel_size-1)/2)**2 + (y-(kernel_size-1)/2)**2)/(2*sigma**2)), (kernel_size, kernel_size))
    return kernel / cp.sum(kernel)

def initialize_synthesized_image(example_img:cp.ndarray, synth_size:Tuple[int,int], patch_size:int=3) -> Tuple[cp.ndarray, cp.ndarray]:
    diff = patch_size // 2

    # Create a blank image to store the synthesized image
    synthesized_img = cp.zeros(synth_size + (example_img.shape[-1],), dtype=cp.float32)
    potential_map = cp.zeros(synth_size, dtype=int)
    #visited_map = cp.zeros(synth_size, dtype=bool)
    ...
    potential_map = cp.max(potential_map) - potential_map + 1
    potential_map[center_i-diff:center_i+diff+1, center_j-diff:center_j+diff+1] = 0

    return synthesized_img, potential_map 

def get_target_patch(synthesized_img:cp.ndarray, potential_map:cp.ndarray, i:int, j:int, kernel_size:int, diff:int) -> Tuple[cp.ndarray, cp.ndarray]:
    # Create a zero matrix to store the target patch
    target_patch = cp.zeros((kernel_size, kernel_size, synthesized_img.shape[-1]))
    patch_mask = cp.zeros((kernel_size, kernel_size))
    ...
    return target_patch, cp.repeat(patch_mask[:,:,cp.newaxis], synthesized_img.shape[-1], axis=2)
        

def find_best_matching_patch(example_patches:cp.ndarray, target_patch:cp.ndarray, patch_mask:cp.ndarray, gaussian_kernel:cp.ndarray, threshold:float = 0.8, attenuation_factor:float=80) -> cp.ndarray:
    # Calculate the squared difference between the target patch and all example patches
    patches = cp.copy(example_patches)
    patches -= target_patch # In place operations is faster than a single line operation
    patches *= patch_mask
    patches **= 2
    patches *= gaussian_kernel
    squared_diffs = cp.sum(patches, axis=(1, 2, 3))

    # Convert the squared differences to probabilities
    prob = 1 - squared_diffs / cp.max(squared_diffs)
    prob *= (prob > threshold*cp.max(prob))  # thresholding the probabilities
    prob = prob**attenuation_factor  # attenuate the probabilities
    prob /= cp.sum(prob)  # normalize the probabilities
    
    # Find the index of the best matching patch
    best_matching_patch_index = cp.random.choice(cp.arange(example_patches.shape[0]), p=prob)
    
    return example_patches[best_matching_patch_index]

def synthesize_texture(example_img:cp.ndarray, synth_size:Tuple[int,int], kernel_size:int, sigma:float=3, patch_size:int=3, threshold:float=0.8, attenuation_factor:float=80) -> cp.ndarray:
    ...
    gaussian_kernel = cp.repeat(create_gaussian_kernel(kernel_size, sigma)[:,:,cp.newaxis], example_img.shape[-1], axis=2)  # create a Gaussian kernel

    # Get the number of non-zero pixels in the potential map
    n_no_pixels = cp.where(potential_map != 0)[0].shape[0]
    ...
    for n_pixel in range(n_no_pixels):
        ...
        i, j = cp.unravel_index(cp.argmax(potential_map), potential_map.shape)  # get the pixel with the highest potential
        ...
        synthesized_img[i, j] = cp.copy(best_matching_patch[diff, diff])
...
if __name__ == '__main__':
    ...
    print(image_path)
    image_array = cp.array(Image.open(image_path))
            ...
            for patch_size in PATCH_SIZES:
                for kernel_size in KERNEL_SIZES:
                    for sigma in SIGMAS:
                        ...
                        plt.imsave(save_path, synthesized_img.get())
